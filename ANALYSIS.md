...
That branch was already non-destructive:
- it correctly detected an existing HyperCode instance
...
- `packages/cli/src/commands/start.ts` already had:
  - `HypercodeAlreadyRunningError`
  - truthful reuse logging for the control plane itself
  - dashboard helpers for the normal startup branch (`pickDashboardPort`, `resolveDashboardUrl`, browser-open logic)
- However, those dashboard helpers were only used after starting a new control plane.
...
#### 2. Improved the `HypercodeAlreadyRunningError` branch
When startup detects an already-running HyperCode instance, it now also:
- prints the detection source:
  - startup lock
  - live port probe
