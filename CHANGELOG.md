...
- **Go-Native Memory Relations & Handoff/Pickup Ownership**: Go now also owns truthful local fallback behavior for memory pivot search, memory timeline windows, cross-session memory links, and direct agent-memory handoff/pickup routes, generating and restoring real portable handoff artifacts from `.hypercode/agent_memory/memories.json` when `/trpc` is unavailable.
...
- **Browser Compat Routed to Go Fallbacks**: The shared Next.js `/api/trpc/[trpc]` compat route now translates the Browser dashboard page onto Go-native `/api/browser/*` endpoints when `/trpc` is unavailable.
- **Near-Total Dashboard Compat Routed to Go Fallbacks**: The shared Next.js `/api/trpc/[trpc]` compat route now translates almost all remaining dashboard clusters—including Swarm, Squad, Marketplace, Links Backlog, Infrastructure, Expert, and Suggestions—onto Go-native endpoints. This allows the vast majority of the dashboard UI to remain truthful and interactive in degraded mode.
- **Council and Director Compat Routed to Go Fallbacks**: The shared Next.js `/api/trpc/[trpc]` compat route now translates the core orchestration surfaces (Council and Director) onto Go-native endpoints, allowing the Autopilot and Director dashboard pages to function in degraded mode.
- **Submodule, Git, and Catalog Compat Routed to Go Fallbacks**: The shared Next.js compat route now translates Knowledge/Architecture (submodule and git) and MCP Registry (catalog) pages to Go-native endpoints.
...
- **Already-Running Dashboard Reuse/Attach Flow**: When `hypercode start` detects an already-running control plane, it now probes for an existing dashboard runtime, reuses/opens it when present, and otherwise starts a dashboard-only runtime attached to the live control plane instead of merely telling the operator to do it manually.
...
- **Go Saved-Context Save Fallback Parity**: Go degraded mode now also owns truthful local fallback behavior for `memory.saveContext`, validating required saved-context fields and persisting new entries into `.hypercode/memory/contexts.json` with generated ids so they immediately participate in local list/query/get/delete behavior when `/trpc` is unavailable.
