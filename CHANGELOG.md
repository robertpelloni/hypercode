...
- **Go MCP Bridge Surface**: The experimental Go workspace now exposes `/api/mcp/status`, `/api/mcp/servers/runtime`, `/api/mcp/servers/configured`, `/api/mcp/servers/registry-snapshot`, `/api/mcp/tools`, `/api/mcp/tools/search`, `/api/mcp/tools/call`, `/api/mcp/tools/schema`, `/api/mcp/preferences`, and `/api/mcp/working-set/*` as bridge routes into the existing TypeScript MCP routers, so the sidecar can surface real MCP runtime, tool, and working-set operations without pretending to own native Go MCP execution.
