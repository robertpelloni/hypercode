...
export default function ConfigEditor() {
    ...
    const configUnavailable = Boolean(configQuery.error)
        || (configQuery.data !== undefined && (typeof configQuery.data !== 'object' || configQuery.data === null || Array.isArray(configQuery.data)));

    useEffect(() => {
        if (!configUnavailable && configQuery.data) {
            setJsonContent(JSON.stringify(configQuery.data, null, 2));
        }
    }, [configQuery.data, configUnavailable]);
        ...
        <div className="p-6 border rounded-lg bg-zinc-900 text-zinc-100 shadow-md w-full max-w-2xl mt-8">
            ...
            {configUnavailable && (
                <div className="mb-4 rounded border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-200">
                    Configuration unavailable: {configQuery.error?.message ?? 'Configuration returned an invalid payload.'}
                </div>
            )}
            <div className="relative">
                <textarea
                    ...
                    disabled={configQuery.isPending || configUnavailable}
                />
            </div>
            <div className="flex justify-between items-center mt-4">
                <span className={`text-sm ${status.includes("Error") ? "text-red-400" : status ? "text-green-400" : "text-zinc-400"}`}>
                    {configQuery.isPending ? 'Loading configuration...' : configUnavailable ? 'Configuration unavailable.' : status}
                </span>
                <button
                    onClick={handleSave}
                    disabled={configQuery.isPending || configUnavailable || updateMutation.isPending}
