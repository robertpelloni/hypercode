...
export async function executeCompatibleRunCode(args, executor, options = {}) {
...
export function createCompatibleAgentRunner(llm, options = {}) {
...
export async function executeCompatibleRunAgent(args, agentRunner, delegatedToolCaller, unavailableMessage) {
...
export async function executeCompatibleSaveMemory(args, agentMemoryService, metadata = { source: 'tool_call' }, unavailableMessage = 'Memory service not available.') {
...
export async function executeCompatibleSearchMemory(args, agentMemoryService, unavailableMessage = 'Memory service not available.') {
    if (!agentMemoryService) {
        return {
            isError: true,
            content: [{ type: 'text', text: unavailableMessage }],
...
export async function executeCompatibleSaveScript(args, savedScriptStore) {
    const script = {
...
export async function executeCompatibleImportConfig(args, configImportService, unavailableMessage) {
...
export async function executeCompatibleRunPython(args, pythonExecutor) {
    const code = typeof args.code === 'string' ? args.code : '';
