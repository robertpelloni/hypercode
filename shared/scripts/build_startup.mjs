...
function runGoPrimaryBuild() {
  ...
  const aiBuild = runPnpm(['-C', 'hyperharness/packages/ts/ai', 'run', 'build']);
  if ((aiBuild.status ?? 1) !== 0) {
    fail('AI startup build failed', aiBuild);
  }

  const agentsBuild = runPnpm(['-C', 'hypercode/packages/ts/agents', 'run', 'build']);
  if ((agentsBuild.status ?? 1) !== 0) {
    fail('Agents startup build failed', agentsBuild);
  }

  const coreBuild = runPnpm(['-C', 'hypercode/packages/ts/core', 'run', 'build']);
  if ((coreBuild.status ?? 1) !== 0) {
    fail('Core startup build failed', coreBuild);
  }

  const cliBuild = runPnpm(['-C', 'hypercode/packages/ts/cli', 'run', 'build']);
  ...
  if (process.env.HYPERCODE_STARTUP_BUILD_WEB === '1') {
    ...
    const webBuild = runPnpm(['-C', 'hypercode/apps/web', 'run', 'build']);
