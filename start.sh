...
if command -v go &>/dev/null; then
    ...
    (cd go && go build -ldflags "-X internal/buildinfo.Version=${VER}" -buildvcs=false -o ../../bin/borg ./cmd/borg 2>/dev/null) && echo "      ✓ bin/borg built" || echo "      [WARN] Go build failed"
else
...
if [ "$BORG_SKIP_BUILD" != "1" ]; then
    pnpm run build:workspace 2>/dev/null || { pnpm -C packages/core exec tsc; pnpm -C packages/cli exec tsc; } || true
fi
